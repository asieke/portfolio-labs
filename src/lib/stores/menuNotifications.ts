...
export const menuNotificationsShowing = writable(false);
...
const menuNotificationsClickHandler = (event: Event) => {
	const target = event.target as HTMLElement;
	if (!target.closest('#menuNotifications')) {
		menuNotificationsShowing.set(false);
	}
};
...
menuNotificationsShowing.subscribe((value) => {
	if (browser) {
		if (value === true) {
			document.addEventListener('click', menuNotificationsClickHandler);
		} else {
			document.removeEventListener('click', menuNotificationsClickHandler);
		}
	}
});

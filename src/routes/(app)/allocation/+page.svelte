<script lang="ts">
	...
	let targetAllocation = portfolios?.[0].asset_class.map((a) => ({
		name: a.name,
		target: a.total / (portfolio?.market_value || 1),
		breakdown: a.breakdown.map((ab) => ({
			name: ab.name,
			target: ab.total / a.total
		}))
	}));
...
<Container layout="right">
	<div slot="left" class="min-h-[400px]">
		<h4>My Asset Allocation</h4>

		{#if display}
			<div class="mb-8 flex h-32 w-full flex-row">
			...
			<div class="flex items-center justify-between rounded-tl-md rounded-tr-md bg-slate-600 px-2 py-2 text-xs dark:bg-slate-700">
				...
				<div class="w-1/12 text-left"><span class="text-white">Date</span></div>
			</div>
			{#each display as row}
				{#each row.breakdown as asset}
					<TableRow label={row.name} {asset} bgcolor={row.color} total={display.reduce((a, b) => a + b.total, 0)} />
				{/each}
				<TableRow label="Total" asset={row} bgcolor={row.color} total={display.reduce((a, b) => a + b.total, 0)} />
			{/each}
		{/if}
	</div>
	<div slot="right">
